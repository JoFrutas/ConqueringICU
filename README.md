...
## Latest update — 0.4.1 · 7 September 2026

The demo now includes **62 questions**, updated presentation and combat animations, **REGULAR / ALTERNATE** soundtracks, and installation/removal options for both platforms. [Read the update log](CHANGELOG.md).
...
- **62 clinical questions** restricted to those castles.
...
The Windows installer and macOS portable ZIP contain the same version 0.4.1 demo; choose the package for your operating system.

| Platform | Package | Official download |
|---|---|---|
| Windows 10/11 | Per-user installer | [Download `Conquering-Critical-Care-Demo-Setup-0.4.1.exe` from MediaFire](https://www.mediafire.com/file/7tsnjfjkb1s0swt/Conquering-Critical-Care-Demo-Setup-0.4.1.exe/file) |
| macOS 11 or later | Portable ZIP for Apple Silicon and Intel | [Download `Conquering-Critical-Care-Demo-Mac-0.4.1.zip` from MediaFire](https://www.mediafire.com/file/yutjian7t3qxnle/Conquering-Critical-Care-Demo-Mac-0.4.1.zip/file) |
...
> **Data note for version 0.4.1:** these public demo packages have no remote analytics service configured and do not transmit gameplay answers. Progress and preferences remain local to the player's browser profile. Any future data-enabled build must be identified and documented separately.
...
To remove it, close the game and choose **Settings → Apps → Installed apps → Conquering Critical Care Demo → Uninstall**, or run **UNINSTALL_DEMO.bat** in the installed folder. Browser progress and preferences are kept.
...
2. Open the extracted `Conquering-Critical-Care-Demo-Mac-0.4.1` folder.
...
This macOS package is not signed or notarized by Apple. If macOS blocks it, Control-click `Open Demo.command`, choose **Open**, and confirm. If required, use **System Settings → Privacy & Security → Open Anyway**. Only do this for the file downloaded from the official link above after verifying its checksum.

For an optional per-user installation, run **Install Demo.command**; the demo is copied to `~/Applications/Conquering Critical Care Demo`. Close the game before running **Uninstall Demo.command** inside the installed folder to move it to the Trash. A portable copy can simply be moved to the Trash after closing its Terminal window. Browser progress is kept.

Windows installation and uninstallation were tested. The Mac archive, runtime hashes, executable permissions and launcher scripts were checked; execution on a real Mac remains untested for this release.
